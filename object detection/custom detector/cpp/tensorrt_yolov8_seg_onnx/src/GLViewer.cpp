...
Simple3DObject createFrustum(sl::CameraParameters const& param) {
...
void GLViewer::init(int argc, char **argv, sl::CameraParameters const& param, bool const isTrackingON) {
    glutInit(&argc, argv);
...
void GLViewer::updateData(sl::Mat const& matXYZRGBA, std::vector<sl::ObjectData> const& objs, sl::Transform const& pose) {
    mtx.lock();
...
void GLViewer::createBboxRendering(std::vector<sl::float3> const& bbox, sl::float4& bbox_clr) {
...
void GLViewer::createIDRendering(sl::float3 const& center, sl::float4 const& clr, unsigned int id) {
    ObjectClassName tmp;
...
Simple3DObject::Simple3DObject(sl::Translation const& position, bool const isStatic) : isStatic_(isStatic) {
    vaoID_ = 0;
...
void Simple3DObject::addBBox(std::vector<sl::float3> const& pts, sl::float4& clr) {
...
void Simple3DObject::addPt(sl::float3 const& pt) {
    vertices_.push_back(pt.x);
    vertices_.push_back(pt.y);
    vertices_.push_back(pt.z);
}

void Simple3DObject::addClr(sl::float4 const& clr) {
...
void Simple3DObject::addPoint(sl::float3 const& pt, sl::float4 const& clr) {
    addPt(pt);
    addClr(clr);
    indices_.push_back((int) indices_.size());
}

void Simple3DObject::addLine(sl::float3 const& p1, sl::float3 const& p2, sl::float4 const& clr) {
    addPoint(p1, clr);
    addPoint(p2, clr);
}

void Simple3DObject::addTriangle(sl::float3 const& p1, sl::float3 const& p2, sl::float3 const& p3, sl::float4 const& clr) {
    addPoint(p1, clr);
    addPoint(p2, clr);
    addPoint(p3, clr);
}

void Simple3DObject::addFullEdges(std::vector<sl::float3> const& pts, sl::float4& clr) {
    clr.w = 0.4f;
...
void Simple3DObject::addVerticalEdges(std::vector<sl::float3> const& pts, sl::float4& clr) {
...
void Simple3DObject::addTopFace(std::vector<sl::float3> const& pts, sl::float4& clr) {
    clr.a = 0.3f;
    for (auto it : pts)
        addPoint(it, clr);
}

void Simple3DObject::addVerticalFaces(std::vector<sl::float3> const& pts, sl::float4& clr) {
...
void Simple3DObject::setDrawingType(GLenum const type) {
    drawingType_ = type;
}
...
void Simple3DObject::translate(sl::Translation const& t) {
    position_ = position_ + t;
}

void Simple3DObject::setPosition(sl::Translation const& p) {
    position_ = p;
}

void Simple3DObject::setRT(sl::Transform const& mRT) {
    position_ = mRT.getTranslation();
    rotation_ = mRT.getOrientation();
}

void Simple3DObject::rotate(sl::Orientation const& rot) {
    rotation_ = rot * rotation_;
}

void Simple3DObject::rotate(sl::Rotation const& m) {
    this->rotate(sl::Orientation(m));
}

void Simple3DObject::setRotation(sl::Orientation const& rot) {
    rotation_ = rot;
}

void Simple3DObject::setRotation(sl::Rotation const& m) {
...
bool Shader::compile(GLuint &shaderId, GLenum const type, GLchar const* src) {
...
void checkError(cudaError_t const& err) {
    if (err != cudaSuccess)
...
void PointCloud::initialize(sl::Resolution const& res) {
...
void PointCloud::pushNewPC(sl::Mat const& matXYZRGBA) {
    if (matGPU_.isInit()) {
...
void PointCloud::draw(sl::Transform const& vp) {
    if (matGPU_.isInit()) {
        glUseProgram(shader.it.getProgramId());
...
CameraGL::CameraGL(sl::Translation const& position, sl::Translation const& direction, sl::Translation const& vertical) {
...
void CameraGL::setProjection(float const horizontalFOV, float const verticalFOV, float const znear, float const zfar) {
...
void CameraGL::setOffsetFromPosition(sl::Translation const& o) {
    offset_ = o;
}
...
void CameraGL::setDirection(sl::Translation const& direction, sl::Translation const& vertical) {
...
void CameraGL::translate(sl::Translation const& t) {
    position_ = position_ + t;
}

void CameraGL::setPosition(sl::Translation const& p) {
    position_ = p;
}

void CameraGL::rotate(sl::Orientation const& rot) {
    rotation_ = rot * rotation_;
    updateVectors();
}

void CameraGL::rotate(sl::Rotation const& m) {
    this->rotate(sl::Orientation(m));
}

void CameraGL::setRotation(sl::Orientation const& rot) {
    rotation_ = rot;
    updateVectors();
}

void CameraGL::setRotation(sl::Rotation const& m) {
    this->setRotation(sl::Orientation(m));
}
