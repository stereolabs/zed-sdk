        ...
        for obj in objects:
            if renderObject(obj, isTrackingON):
                ...
                if obj.mask.is_init():
                    ...
                    mask = cv2.resize(obj.mask.numpy(), (roi_height, roi_width), interpolation=cv2.INTER_LINEAR)
                    overlay_shift = (int(top_left_corner[0]), int(top_left_corner[1]))
                    overlay_roi[mask != 0] = base_color
                    draw_mask(left_display, mask, base_color, overlay_shift)
                else:
