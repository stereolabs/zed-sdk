...
void GLViewer::updateData(sl::Mat &matXYZRGBA, sl::Objects &objs, sl::Bodies & bodies, sl::Transform& pose) {
    mtx.lock();
    ...
    for(auto &obj: objs.object_list){
        if (canRender(obj.tracking_state, isTrackingON_)) {
            auto bb_ = obj.bounding_box;
            if (!bb_.empty()) {
                auto clr_class = getColorClass((int) obj.label);
                auto clr_id = generateColorID_f(obj.id);
    ...
    for (auto& body : bodies.body_list) {
        if (canRender(body.tracking_state, isTrackingON_)) {
            auto clr_id = generateColorID_f(body.id);
            createIDRendering(body.position, clr_id, body.id);
            auto clr_bones = generateColorID_f(body.id);
            if (bodies.body_format == sl::BODY_FORMAT::BODY_18)
                createSKPrimitive(body, sl::BODY_18_BONES, skeletons, clr_id);
            if (bodies.body_format == sl::BODY_FORMAT::BODY_34)
