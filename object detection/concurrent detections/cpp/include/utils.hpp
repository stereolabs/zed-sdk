...
inline bool canRender(const sl::OBJECT_TRACKING_STATE tracking_state, const bool isTrackingON) {
    if (isTrackingON)
        return (tracking_state == sl::OBJECT_TRACKING_STATE::OK);
    else
        return (tracking_state == sl::OBJECT_TRACKING_STATE::OK || tracking_state == sl::OBJECT_TRACKING_STATE::OFF);
}
