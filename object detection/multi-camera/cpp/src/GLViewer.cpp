...
void GLViewer::addObject(sl::ObjectData &obj, Simple3DObject &simpleObj, sl::float3 clr_id, bool raw) {
    simpleObj.addBBox(obj.bounding_box, clr_id);
}
...
void GLViewer::updateObjects(sl::Objects &objects, std::map<sl::CameraIdentifier, sl::Objects>& singldata, sl::FusionMetrics& metrics) {
    mtx.lock();
    ...
    if (objects.is_new) {
        ...
        for(auto &it:objects.object_list) {
            ...
            if (renderObject(it, objects.is_tracked))
            {
                addObject(it, skeletons, colors_sk[it.id], false);
            }
        }
    ...
    for (auto &it : singldata) {
        ...
        if (it.second.is_new) 
        {
            ...
            for (auto& obj : it.second.object_list) {
                if(renderObject(obj, it.second.is_tracked))
                { 
                    addObject(obj, sk_r, clr, true);
                }
            }
        }
