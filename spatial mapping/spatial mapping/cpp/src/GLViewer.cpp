...
void GLViewer::render() {
    if(available) {
        ...
        //printText();
#if 0
        cv::Mat img = get_ocv_img_from_gl_img(0);
        ostringstream filename1;
        filename1 << "./left" << setfill('0') << setw(6) << svo_position << ".png";
        cv::imwrite(filename1.str(), img);
#endif
...
cv::Mat GLViewer::get_ocv_img_from_gl_img(GLuint ogl_texture_id)
{

    GLenum gl_texture_width    = glutGet(GLUT_WINDOW_WIDTH);
    GLenum gl_texture_height  = glutGet(GLUT_WINDOW_HEIGHT);

    std::cout<<" Size : "<<gl_texture_width<<" , "<<gl_texture_height<<std::endl;

    unsigned char* gl_texture_bytes = (unsigned char*) malloc(sizeof(unsigned char)*gl_texture_width*gl_texture_height*4);
    glReadPixels(0, 0, gl_texture_width, gl_texture_height, GL_RGBA, GL_UNSIGNED_BYTE, gl_texture_bytes);

    cv::Mat tmp =  cv::Mat(gl_texture_height, gl_texture_width, CV_8UC4, gl_texture_bytes);
