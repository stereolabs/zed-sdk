...
{
#ifdef COMPILE_WITH_ZEDHUB
    sl_hub::STATUS_CODE exit_sl_hub_status = sl_hub::HubClient::disconnect();
    if (exit_sl_hub_status != sl_hub::STATUS_CODE::SUCCESS)
    {
        std::cout << "[ZedHub][ERROR] Terminate with error " << exit_sl_hub_status << std::endl;
        exit(EXIT_FAILURE);
    }
#endif
}
