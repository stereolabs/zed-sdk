# This is a modified version of the viewer.py that uses GLFW instead of GLUT
...
class Simple3DObject:
    ...
    def push_to_GPU(self):
        ...
        type_draw = GL_STATIC_DRAW if self.is_static else GL_DYNAMIC_DRAW
    ...
    def init(self, res):
        ...
        # Vertices
        glBindBuffer(GL_ARRAY_BUFFER, self.vboID[0])
        glBufferData(GL_ARRAY_BUFFER, self.elementbufferSize * self.pt_type * 4, None, type_draw)
        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, self.pt_type, GL_FLOAT, GL_FALSE, 0, None)
        ...
        if self.clr_type:
            glBindBuffer(GL_ARRAY_BUFFER, self.vboID[1])
            glBufferData(GL_ARRAY_BUFFER, self.elementbufferSize * self.clr_type * 4, None, type_draw)
            glEnableVertexAttribArray(1)
            glVertexAttribPointer(1, self.clr_type, GL_FLOAT, GL_FALSE, 0, None)

        # Create indices
        for i in range(self.elementbufferSize):
    ...
    def setPoints(self, pc):
        ...
        # Update buffer with point cloud data
        glBindBuffer(GL_ARRAY_BUFFER, self.vboID[0])
        glBufferSubData(GL_ARRAY_BUFFER, 0, self.elementbufferSize * self.pt_type * 4, 
...
class GLViewer:
    ...
    def init(self, _argc, _argv, res):
        ...
        wnd_w = int(mode.size.width * 0.9)
        wnd_h = int(mode.size.height * 0.9)
        ...
        self.window = glfw.create_window(wnd_w, wnd_h, "ZED Depth Sensing", None, None)
        ...
        opengl_version = glGetString(GL_VERSION)  # Get OpenGL version
        if opengl_version:
            print("OpenGL Version:", opengl_version.decode())  # Decode from bytes
        else:
            print("ERROR: OpenGL Version could not be retrieved!")
    ...
    def on_mouse_button(self, window, button, action, mods):
        if button == glfw.MOUSE_BUTTON_LEFT:
            self.mouse_button[0] = action == glfw.PRESS
        elif button == glfw.MOUSE_BUTTON_RIGHT:
            self.mouse_button[1] = action == glfw.PRESS
    ...
    def on_mouse_move(self, window, xpos, ypos):
        if self.mouse_button[0] or self.mouse_button[1]:
            self.mouseMotion[0] = xpos - self.previousMouseMotion[0]
            self.mouseMotion[1] = ypos - self.previousMouseMotion[1]
            self.previousMouseMotion = [xpos, ypos]

    def on_scroll(self, window, xoffset, yoffset):
        self.wheelPosition = yoffset
    ...
    def render(self):
        if self.available and not glfw.window_should_close(self.window):
            glfw.poll_events()
