...
namespace sl
{
    ...
    class GLViewer
    {
        public GLViewer()
        {
            ...
            skeleton_raw = new Simple3DObject();

            pointClouds = new Dictionary<ulong, PointCloud>();
            poses = new Dictionary<ulong, Matrix4x4>(); 
            mtx = new Mutex();
        }
        ...
        public void Init(bool showOnlyOk)
        {
            ...
            skeleton_raw.init();
            skeleton_raw.setDrawingType(PrimitiveType.Quads);
        ...
        public void InitPointCloud(ulong id, Resolution res)
        {
            mtx.WaitOne();
            if (!pointClouds.ContainsKey(id))
            {
                PointCloud pc = new PointCloud();
                pc.initialize(res);
                pointClouds.Add(id, pc);
            }
            mtx.ReleaseMutex();
        }

        void CreateSK<T>(ref Simple3DObject obj, Tuple<T, T>[] Bones, Vector3[] keypoints, float4 clr_id, bool raw = false)
        {
            foreach (var limb in Bones)
            {
                ...
                {
                    obj.addCylinder(new float3(kp_1.X, kp_1.Y, kp_1.Z), new float3(kp_2.X, kp_2.Y, kp_2.Z), clr_id, raw ? 0.005f : 0.01f);
                }
            }
        }

        public void SetCameraPose(ulong id, sl.Pose pose)
        {
            mtx.WaitOne();

            Matrix4x4 cam_pose = Matrix4x4.Identity;
        ...
        public void UpdatePointCloud(ulong id, sl.Mat pc)
        {
            mtx.WaitOne();
            foreach (var pointCloud in pointClouds)
            {
                if (pointClouds.ContainsKey(id))    pointClouds[id].pushNewPC(pc);
            }
            mtx.ReleaseMutex();
        }

        public void UpdateBodies(Bodies bodies, Dictionary<ulong, sl.Bodies> singleData)
        {
            skeleton.clear();

            // For each object
            for (int idx = 0; idx < bodies.nbBodies; idx++)
            {
                sl.BodyData bod = bodies.bodiesList[idx];
                Vector3[] keypoints = bod.keypoints;
                float4 clr_id = generateColorID(bod.id);
                // Only show tracked objects
                if (renderObject(bod, showOnlyOK_))
                {
                    if (bodies.bodyFormat == BODY_FORMAT.BODY_38)
                    {
                        CreateSK(ref skeleton, SKELETON_BONES_BODY_38, keypoints, clr_id);

                        for (int i = 0; i < (int)BODY_38_PARTS.LAST; i++)
                        {
                            Vector3 kp = bod.keypoints[i];
                    ...
                    else if (bodies.bodyFormat == BODY_FORMAT.BODY_34)
                    {
                        CreateSK(ref skeleton, SKELETON_BONES_BODY_34, keypoints, clr_id);

                        for (int i = 0; i < (int)BODY_34_PARTS.LAST; i++)
                        {
                            Vector3 kp = bod.keypoints[i];
                            float norm = kp.Length();
                            if (!float.IsNaN(norm) && norm > 0)
                    ...
                    else if (bodies.bodyFormat == BODY_FORMAT.BODY_18)
                    {
                        CreateSK(ref skeleton, SKELETON_BONES_BODY_18, keypoints, clr_id);

                        for (int i = 0; i < (int)BODY_18_PARTS.LAST; i++)
                        {
                            Vector3 kp = bod.keypoints[i];
                            float norm = kp.Length();
                            if (!float.IsNaN(norm) && norm > 0)
            ...
            {
                sl.Bodies body = single.Value;
                for (int idx = 0; idx < body.nbBodies; idx++)
                {
                    sl.BodyData bod = body.bodiesList[idx];
                    Vector3[] keypoints = bod.keypoints;
                    float4 clr_id = generateColorID(bod.id);
                    // Only show tracked objects
                    if (renderObject(bod, false))
                    {
                        if (body.bodyFormat == BODY_FORMAT.BODY_38)
                        {
                            CreateSK(ref skeleton_raw, SKELETON_BONES_BODY_38, keypoints, clr_id, true);
                        }
                        else if (body.bodyFormat == BODY_FORMAT.BODY_34)
                        {
                            CreateSK(ref skeleton_raw, SKELETON_BONES_BODY_34, keypoints, clr_id, true);
                        }
                        else if (body.bodyFormat == BODY_FORMAT.BODY_18)
                        {
                            CreateSK(ref skeleton_raw, SKELETON_BONES_BODY_18, keypoints, clr_id, true);
                        }
                    }
                }
            }
        ...
        public void keyEventFunction(NativeWindowKeyEventArgs e)
        {
            ...
            if (e.Key == KeyCode.R)
            {
                showRawSkeletons = !showRawSkeletons;
            }
        }
        ...
        public void draw()
        {
            ...
            if (showPC)
            {
                ...
                {
                    Matrix4x4 vpMatrixCam = vpMatrix * pose.Value;
                    if (pointClouds.ContainsKey(pose.Key))
                    {
                        pointClouds[pose.Key].draw(vpMatrixCam);
                    }
                }
            }
            ...
            Gl.UniformMatrix4f(shaderSK.MVP_Mat, 1, true, vpMatrix);
            skeleton.draw();
            if (showRawSkeletons) skeleton_raw.draw();
            Gl.UseProgram(0);
        ...
        bool renderObject(BodyData i, bool showOnlyOK)
        {
            if (showOnlyOK)
        ...
        private static readonly Tuple<sl.BODY_18_PARTS, sl.BODY_18_PARTS>[] SKELETON_BONES_BODY_18 =
{
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.NOSE, BODY_18_PARTS.NECK),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.NECK, BODY_18_PARTS.RIGHT_SHOULDER),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_SHOULDER, BODY_18_PARTS.RIGHT_ELBOW),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_ELBOW, BODY_18_PARTS.RIGHT_WRIST),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.NECK, BODY_18_PARTS.LEFT_SHOULDER),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.LEFT_SHOULDER, BODY_18_PARTS.LEFT_ELBOW),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.LEFT_ELBOW, BODY_18_PARTS.LEFT_WRIST),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_HIP, BODY_18_PARTS.RIGHT_KNEE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_KNEE, BODY_18_PARTS.RIGHT_ANKLE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.LEFT_HIP, BODY_18_PARTS.LEFT_KNEE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.LEFT_KNEE, BODY_18_PARTS.LEFT_ANKLE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_SHOULDER, BODY_18_PARTS.LEFT_SHOULDER),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_HIP, BODY_18_PARTS.LEFT_HIP),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.NOSE, BODY_18_PARTS.RIGHT_EYE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.RIGHT_EYE, BODY_18_PARTS.RIGHT_EAR),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.NOSE, BODY_18_PARTS.LEFT_EYE),
        Tuple.Create<BODY_18_PARTS, BODY_18_PARTS>(BODY_18_PARTS.LEFT_EYE, BODY_18_PARTS.LEFT_EAR)
        };

        private static readonly Tuple<sl.BODY_34_PARTS, sl.BODY_34_PARTS>[] SKELETON_BONES_BODY_34 =
    {
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.PELVIS, BODY_34_PARTS.NAVAL_SPINE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.NAVAL_SPINE, BODY_34_PARTS.CHEST_SPINE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.CHEST_SPINE, BODY_34_PARTS.LEFT_CLAVICLE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_CLAVICLE, BODY_34_PARTS.LEFT_SHOULDER),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_SHOULDER, BODY_34_PARTS.LEFT_ELBOW),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_ELBOW, BODY_34_PARTS.LEFT_WRIST),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_WRIST, BODY_34_PARTS.LEFT_HAND),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_HAND, BODY_34_PARTS.LEFT_HANDTIP),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_WRIST, BODY_34_PARTS.LEFT_THUMB),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.CHEST_SPINE, BODY_34_PARTS.RIGHT_CLAVICLE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_CLAVICLE, BODY_34_PARTS.RIGHT_SHOULDER),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_SHOULDER, BODY_34_PARTS.RIGHT_ELBOW),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_ELBOW, BODY_34_PARTS.RIGHT_WRIST),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_WRIST, BODY_34_PARTS.RIGHT_HAND),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_HAND, BODY_34_PARTS.RIGHT_HANDTIP),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_WRIST, BODY_34_PARTS.RIGHT_THUMB),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.PELVIS, BODY_34_PARTS.LEFT_HIP),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_HIP, BODY_34_PARTS.LEFT_KNEE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_KNEE, BODY_34_PARTS.LEFT_ANKLE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_ANKLE, BODY_34_PARTS.LEFT_FOOT),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.PELVIS, BODY_34_PARTS.RIGHT_HIP),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_HIP, BODY_34_PARTS.RIGHT_KNEE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_KNEE, BODY_34_PARTS.RIGHT_ANKLE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_ANKLE, BODY_34_PARTS.RIGHT_FOOT),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.CHEST_SPINE, BODY_34_PARTS.NECK),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.NECK, BODY_34_PARTS.HEAD),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.HEAD, BODY_34_PARTS.NOSE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.NOSE, BODY_34_PARTS.LEFT_EYE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_EYE, BODY_34_PARTS.LEFT_EAR),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.NOSE, BODY_34_PARTS.RIGHT_EYE),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_EYE, BODY_34_PARTS.RIGHT_EAR),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_ANKLE, BODY_34_PARTS.LEFT_HEEL),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_ANKLE, BODY_34_PARTS.RIGHT_HEEL),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.LEFT_HEEL, BODY_34_PARTS.LEFT_FOOT),
        Tuple.Create<BODY_34_PARTS, BODY_34_PARTS>(BODY_34_PARTS.RIGHT_HEEL, BODY_34_PARTS.RIGHT_FOOT)
        };
        ...
        Dictionary<ulong, Matrix4x4> poses;
        Dictionary<ulong, PointCloud> pointClouds;
    ...
    class ImageHandler
    {
        ...
        public void initialize()
        {
            ...
            Gl.TexImage2D(TextureTarget.Texture2d, 0, InternalFormat.Rgba, (int)resolution.width, (int)resolution.height, 0, OpenGL.PixelFormat.Rgba, PixelType.UnsignedByte, null);
        ...
        public void pushNewImage(Mat zedImage)
        {
            ...
            Gl.TexSubImage2D(TextureTarget.Texture2d, 0, 0, 0, zedImage.GetWidth(), zedImage.GetHeight(), OpenGL.PixelFormat.Rgba, PixelType.UnsignedByte, zedImage.GetPtr());
        }
    ...
    class Simple3DObject
    {
        ...
        public void addCylinder(float3 startPosition, float3 endPosition, float4 clr, float radius = 0.010f)
        {
